...
const RequestTracking = () => {
  ...
  return (
    <div className="p-4 sm:p-6 bg-gray-50 m-4 sm:m-6 rounded-lg shadow-lg max-w-full">
          ...
          <thead className="bg-[#d0d9f7] text-black">
            <tr>
              {["Complainer Name", "Request Name", "Description", "Request Date", "Unit Number", "Priority", "Status", "Action"].map((header, idx) => (
                <th key={idx} className="py-2 px-2 sm:px-4 text-nowrap font-semibold text-gray-600 text-center">
                  {header}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {complaintData.map((entry, index) => (
              <tr key={index} className="border-b border-gray-200 ">
                ...
                <td className="py-3 px-2 sm:px-4 text-gray-700 text-center text-nowrap">{entry.requestName}</td>
                <td className="py-3 px-2 sm:px-4 text-gray-700 text-center text-nowrap">{entry.description}</td>
                <td className="py-3 px-2 sm:px-4 text-gray-700 text-center text-nowrap">{entry.date}</td>
