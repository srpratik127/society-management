...
const getExpenseById = async (req, res) => {
  try {
    const expense = await ExpenseDetails.findById(req.params.id).populate(
      "societyId userId"
    );
    if (!expense) {
  ...
  try {
    ...
    if (req.file) {
      ...
      if (existingExpense && existingExpense.bill) {
        const publicId = existingExpense.bill
          .split("/")
          .slice(-2)
          .join("/")
          .split(".")[0];
...
const deleteExpense = async (req, res) => {
  try {
    const deletedExpense = await ExpenseDetails.findByIdAndDelete(
      req.params.id
    );
    if (!deletedExpense) {
      return res.status(404).json({ message: "Expense not found" });
...
const getTotalExpense = async (req, res) => {
  try {
    const totalExpense = await ExpenseDetails.aggregate([
      {
        $group: {
          _id: null,
